...
from flask import url_for,render_template,redirect,request,flash,get_flashed_messages,send_from_directory
...
def reg():
    ...
    if username == '' or password == '':
        return redirect_with_msg('/regloginpage/',u'用户名或密码不能空','reglogin')

    user = User.query.filter_by(username = username).first()
    if user != None:
        return redirect_with_msg('/regloginpage/',u'用户名已经存在','reglogin')

    salt= '.'.join(random.sample('ahsfhasdjfh34134',5))
    m= hashlib.md5()
